...
func i32Load(vm *VirtualMachine) {
	base := memoryBase(vm)
	vm.OperandStack.Push(uint64(binary.LittleEndian.Uint32(vm.Mem.Mem[base:])))
}

func i64Load(vm *VirtualMachine) {
	base := memoryBase(vm)
	vm.OperandStack.Push(binary.LittleEndian.Uint64(vm.Mem.Mem[base:]))
}
...
func i32Load8s(vm *VirtualMachine) {
	base := memoryBase(vm)
	vm.OperandStack.Push(uint64(vm.Mem.Mem[base]))
}
...
func i32Load16s(vm *VirtualMachine) {
	base := memoryBase(vm)
	vm.OperandStack.Push(uint64(binary.LittleEndian.Uint16(vm.Mem.Mem[base:])))
}
...
func i32Store(vm *VirtualMachine) {
	...
	base := memoryBase(vm)
	binary.LittleEndian.PutUint32(vm.Mem.Mem[base:], uint32(val))
}

func i64Store(vm *VirtualMachine) {
	val := vm.OperandStack.Pop()
	base := memoryBase(vm)
	binary.LittleEndian.PutUint64(vm.Mem.Mem[base:], val)
}
...
func f64Store(vm *VirtualMachine) {
	...
	base := memoryBase(vm)
	binary.LittleEndian.PutUint64(vm.Mem.Mem[base:], v)
}
...
func i32Store16(vm *VirtualMachine) {
	...
	base := memoryBase(vm)
	binary.LittleEndian.PutUint16(vm.Mem.Mem[base:], v)
}
...
func i64Store32(vm *VirtualMachine) {
	...
	base := memoryBase(vm)
	binary.LittleEndian.PutUint32(vm.Mem.Mem[base:], v)
}

func memorySize(vm *VirtualMachine) {
	vm.ActiveContext.PC++
	vm.OperandStack.Push(uint64(int32(len(vm.Mem.Mem) / vmPageSize)))
}
...
func memoryGrow(vm *VirtualMachine) {
	...
	if vm.InnerModule.SecMemory[0].Max != nil &&
		uint64(n+uint32(len(vm.Mem.Mem)/vmPageSize)) > uint64(*(vm.InnerModule.SecMemory[0].Max)) {
		v := int32(-1)
		vm.OperandStack.Push(uint64(v))
		return
	}

	vm.OperandStack.Push(uint64(len(vm.Mem.Mem)) / vmPageSize)
	// vm.Mem.Memory = append(vm.Mem.Memory, make([]byte, n*vmPageSize)...)
	vm.Mem.GrowTo(int(n))
}
